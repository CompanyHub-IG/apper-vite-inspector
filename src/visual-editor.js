...
export function VisualEditor() {
    ...
    const sendMessage = (message) => {
        CONFIG.ALLOWED_ORIGINS.forEach(origin => {
            try {
                ...
                // Create a serializable copy of the message
                ...
                const serializableMessage = JSON.parse(JSON.stringify(message, (key, value) => {
                    ...
                    if (typeof value === 'object' && value !== null) {
                        ...
                        if (key && (key.startsWith('__react') || key.startsWith('_react') || key.includes('$'))) {
                            return undefined;
                        }
                ...
                window.parent.postMessage(serializableMessage, origin);
            } catch (error) {
    ...
    const initializeElementSelector = () => {
        ...
        const getElementsByLocation = location => {
            console.log("getElementsByLocation", location)
            ...
            try {
                ...
                switch (event.data.type) {
                    ...
                    case "EDIT_TEXT_REQUESTED": {
                        console.log("edit text requested", event.data.payload)
                        const { id, filePath, lineNumber, col } = event.data.payload;
                        getElementsByLocation({
                            filePath,
                            lineNumber,
                            col
        ...
        const handleClick = event => {
            ...
            if (selector.isActive && hasLovId(event.target) && 
                ...
                if (selector.hoveredElement) {
                    ...
                    // const { filePath, lineNumber, col } = parseLovId(elementData.id);
                    // console.log("id", filePath, lineNumber, col)
                    ...
                    //     element.focus();
                    //     const handleInput = () => {
                    //         console.log("input event", element.innerText)
                    //         sendMessage({
                    //             type: "ELEMENT_TEXT_UPDATED",
                    //             payload: {
                    //                 //id,
